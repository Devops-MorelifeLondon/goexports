...
export default function GetInTouchForm() {
  ...
  const inputFields = [
    { name: "name", label: "Full Name", type: "text", placeholder: "John Doe", icon: "M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 3a4 4 0 110 8 4 4 0 010-8z", fullWidth: false },
    ...
    { name: "company", label: "Company Name", type: "text", placeholder: "Acme Inc.", icon: "M3 21h18M3 7v14M21 7v14M6 11h.01M6 15h.01M10 11h.01M10 15h.01M14 11h.01M14 15h.01M18 11h.01M18 15h.01M6 7l6-4 6 4", fullWidth: true },
  ];
      ...
      <div className="relative bg-white/10 backdrop-blur-md rounded-xl p-6 shadow-[0_20px_60px_rgba(0,0,0,0.3)] overflow-hidden border border-white/20">
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 animate-in fade-in duration-300">
                ...
                <div key={field.name} className={`flex flex-col gap-1 ${field.fullWidth ? 'w-full' : 'w-[calc(50%-6px)]'}`}>
                  ...
                  <div className={`relative flex items-center rounded-lg border-[1.5px] transition-all duration-200 bg-black/20 ${
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-2 text-[13px] text-white placeholder-white/40 outline-none bg-transparent"
                    />
                  </div>
                </div>
              ))}
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-white text-black font-bold text-[14px] rounded-lg border-none cursor-pointer transition-transform duration-200 hover:bg-gray-200 hover:shadow-lg active:scale-95 mt-1 flex items-center justify-center gap-2"
            >
              Submit Enquiry
